...
export default function FilterAbsences({
  ...
  const totalAbsenceDays = useMemo(
    () => absences.reduce((acc, absence) => acc + absence.days, 0),
    [absences],
  );
  const { t } = useLanguage();
  return (
    <section className="flex flex-col sm:flex-row justify-between">
      <div className="AbsenceUserFilter flex flex-wrap  justify-between sm:flex-nowrap items-center w-fit text-[#244B77]">
        {hasFilters ? (
          ...
          >
            <FunnelX
              className="bg-red-500 py-1 rounded text-sm font-semibold hover:shadow-lg text-white"
              aria-hidden="true"
            />
          </button>
        ) : (
        ...
        <Dropdown
          ...
          selectedValue={(employee) =>
            employee ? employee.username : t.allEmployees
          }
        ...
        <Dropdown
          ...
          selectedValue={(absence) => (absence ? absence : t.allAbsences)}
          onSelect={handleSelectedAbsence}
        />

        <div className="text-sm font-medium border-b-2 border-b-[#6C99CB] px-2 py-1 text-slate-700">
          Days Off:{" "}
          <strong className="text-[#244B77]">{totalAbsenceDays}</strong>
        </div>
      </div>
      <DateFilter
